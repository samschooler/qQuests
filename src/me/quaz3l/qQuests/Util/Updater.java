...
public class Updater 
{
	...
	private String updateFolder = "/qQuests/update"; // The folder that downloads will be placed in
	...
	public static void start(final Plugin plugin, final String slug, final File file)
	{
	...
	private void saveFile(File folder, String file, String u)
	{
		...
		try
		{
			...
			if(announce) plugin.getLogger().info("Finished downloading.");
			installPlugin(folder, folder.getAbsolutePath() + "/" + file);
		}
		catch (Exception ex)
		{
	...
	private boolean versionCheck(String title)
	{
		if(type != UpdateType.NO_VERSION_CHECK)
		{
			...
			{
				String remoteVersion = title.split(" #")[1].split(" ")[0]; // Get the newest file's version number
	...
	private boolean installPlugin(File uFolder, String updateFile) {
		try {
			Chat.logger("info", "Installing...");
			FileInputStream lin = new FileInputStream(updateFile);
			FileOutputStream lfout;
			lfout = new FileOutputStream(qQuests.plugin.getDataFolder().getPath()+".jar");

			byte[] buffer = new byte[1024];

			int length;
			//copy the file content in bytes 
			while ((length = lin.read(buffer)) > 0){

				lfout.write(buffer, 0, length);
